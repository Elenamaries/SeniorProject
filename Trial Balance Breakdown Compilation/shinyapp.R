...
devtools::install_github("pridiltal/staplr")
  ...
  ui <- fluidPage(
    # Application title
    h1(id='header1', "Trial Balance Breakdown"),
    tags$style(HTML("#header1{color:#007fad;margin:15px;text-align:center; padding:15px;"), 
    style = "background-color:#90ad00; padding:15px; text-align:center"),
    
    fluidRow(
      column(12,
             p("Uploaded files must meet the following criteria: "),
             p("- .xlsx format"),
             p("- Column Order: AccountNum, AccountName, Year1, Year2"),
             p (" "),
             p("Start by clicking the 'Browse' button, and select the Trial Balance you would like to break down "),
             p("This will result in the display of the input file with a few changes (including new TaxAmount and TaxLine columns)"),
             p("To download the file displayed, click the 'Download Excel' button"),
             p("To download the file filled into a Tax Return, click the 'Download PDF' button"),
             p("Dow not click either 'Download' button until after you have uploaded a file and it has displayed"),
             style = "background-color:#90ad00; padding:15px; text-align:center; color: white;")
    ),
    ...
    style = "background-color:#90ad00; padding:30px;")),
    
    h6(id = 'header1', "Created by Elena Juarez"),
   ...
   downloadButton("downloadPDFData", "Download PDF File")
  )
  ...
  server = function(input, output, session) {
    datasetInput <- eventReactive(input$FileInput, {
      ...
      data <- read_xlsx(infile$datapath,  skip = 3)
      
      # Renaming the columns
      names(data) <- c("AccountNum", "AccountName", "Beginning", "End")
      ...
      # Get fields of the TaxPacket PDF
      fields = staplr::get_fields("TaxPacket.pdf")
      ...
      UpdatedTaxPacket <- staplr::set_fields("TaxPacket.pdf", 'UpdatedTaxPacket.pdf', fields)
      
      return(UpdatedTaxPacket)
    ...
    output$downloadData <- downloadHandler(
      ...
      content = function(file) write_xlsx(finalform, file),  
      contentType = "text/excel" )
    
  
    
    
    output$downloadPDFData <- downloadHandler(
     filename = "downloaded_pdf_data.pdf",
     content = function(file) {
       file.copy("UpdatedTaxPacket.pdf", file)
     } )
